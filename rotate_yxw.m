function [varargout] = rotate_yxw(axis, angle, system, varargin)
%align set alignment of the objects in the block
% 
% modified by Xingwang Yong to take another input 'system'
%   [...] = rotate(axis, angle, system, obj <, obj> ...);
%
% The following are the original document
% 
...
if ~isfield(system,'maxGrad') || ~isfield(system,'maxSlew') 
    error('Syntax error, the 3rd input should be mr.opts()!')
end
% warning('You are using the rotate() modified by Xingwang Yong, syntax changed!')
...
if (length(rotated1)>1)
    g{1}=mr.addGradients(rotated1,'system',system);
else
    if (~isempty(rotated1))
        g{1}=rotated1{1};
    end
...
if (length(rotated2)>1)
    g{2}=mr.addGradients(rotated2,'system',system);
else
    if (~isempty(rotated2))
        g{2}=rotated2{1};
    end
